...
- `electron/` — the app
  ...
  - `src/main/` — Electron main: window, `desktopCapturer` sources, 120Hz cursor poller + uiohook global clicks/keys, bundle save, ffmpeg export pipe + audio filters, whisper transcription
...
- iPhone/iPad wired capture (continuity camera enumerate → `getUserMedia`), device-frame bezel, and **Detect touches** autofocus — frame-diff motion centroid tracking, since iOS taps don't reach the global hook
...
- Notarization/signing, auto-update
- Windows/Linux ports
